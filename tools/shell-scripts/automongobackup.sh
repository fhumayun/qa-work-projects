...
MAILADDR="fhumayun@groupcaretech.com"
...
sudo mkdir -p $BACKUPDIR/{daily,weekly,monthly} || shellout 'failed to create directories'

if [ "$LATEST" = "yes" ]; then
    sudo rm -rf "$BACKUPDIR/latest"
    sudo mkdir -p "$BACKUPDIR/latest" || shellout 'failed to create directory'
fi
...
# Weekly Backup
    ...
    sudo rm -f $BACKUPDIR/weekly/week.$REMW.*
    echo
